...
expName = 'PRACTICE_WMpsilocybin_v1'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    if dataDir is None:
        dataDir = _thisDir
    filename = u'data/sub-%s/PRACTICE/sub-%s_%s_%s' % (expInfo['participant'], expInfo['participant'], expName, expInfo['date'])
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/cnj678/Documents/GitHub/WM_Psilocybin/PRACTICE_WMpsilocybin_v1_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # assign participant number number from psychopy input
    ...
    practice_foldername = _os.path.join(sub_foldername, 'PRACTICE')
    ...
    # create the practice folder if it does not already exist
    if not _os.path.exists(practice_foldername):
        _os.makedirs(practice_foldername)
        print(f'PRACTICE folder successfully created for sub-{participant_number}!')
    else:
        print(f'PRACTICE folder already exists for sub-{participant_number}...')
    ...
    dataPractice_foldername = _os.path.join(data_foldername, 'PRACTICE')
    
    if not _os.path.exists(dataPractice_foldername):
        _os.makedirs(dataPractice_foldername)
        print(f'PRACTICE data subfolder successfully created for sub-{participant_number}')
    else:
        print(f'PRACTICE data subfolder already exists for sub-{participant_number}')
    ...
    end_text = visual.TextStim(win=win, name='end_text',
        text='Congratulations, you are all done with the practice for this part of the experiment!\n\n<press SPACE to exit>',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    # -----------------------------
    if _os.path.exists(practice_csv):
        print(f"Design matrices already exist for sub-{participant_number} PRACTICE. Skipping generation.")
    else:
        print(f"Generating design matrices for sub-{participant_number} PRACTICE...")
