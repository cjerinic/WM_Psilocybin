...
expName = 'WMpsilocybin_v6'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    if dataDir is None:
        dataDir = _thisDir
    filename = u'data/sub-%s/ses-%s/sub-%s_ses-%s_%s_%s' % (expInfo['participant'], expInfo['session'], expInfo['participant'], expInfo['session'], expName, expInfo['date']) 
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/cnj678/Documents/GitHub/WM_Psilocybin/WMpsilocybin_v6_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # assign participant number/session number from psychopy input
    ...
    # create subject folder, then session folder, then phase folders inside session
    sub_foldername = _os.path.join(expDir, 'subject_designs', f'sub-{participant_number}')
    ses_foldername = _os.path.join(sub_foldername, f'ses-{session_number}')
    
    # phase folders to create (nested under session)
    phase_subFolder = ['practice', 'main']
    
    # create the subject folder if it does not already exist
    if not _os.path.exists(sub_foldername):
        _os.makedirs(sub_foldername)
    ...
    if not _os.path.exists(ses_foldername):
        _os.makedirs(ses_foldername)
        print(f'Session folder successfully created for ses-{session_number}!')
    else:
        print(f'Session folder already exists for ses-{session_number}...')
    
    # create each phase subfolder within the session folder
    for subfolder in phase_subFolder:
        phase_folder_path = _os.path.join(ses_foldername, subfolder)
    ...
    if not _os.path.exists(data_foldername):
        ...
        print(f'Data folder already exists for sub-{participant_number}...')
    
    # add subfolder for session
    dataSes_foldername = _os.path.join(data_foldername, f'ses-{session_number}')
    
    if not _os.path.exists(dataSes_foldername):
        _os.makedirs(dataSes_foldername)
        print(f'session subfolder successfully created for ses-{session_number}')
    else:
        print(f'session subfolder already exists for ses-{session_number}')
    ...
    # --- Initialize components for Routine "selfStart_nextTrial" ---
    selfStart_instructText = visual.TextStim(win=win, name='selfStart_instructText',
        text='After you report all the colored dot locations (all 5 colors below the circle have become transparent), you will be able to start the next trial on your own. To do this, you will use the mouse to click on the black fixation square in the center of the circle.\n\nThe self-start procedure will look like this:',
        font='Arial',
        pos=(0, 0.2), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    selfStart_instructImage = visual.ImageStim(
        win=win,
        name='selfStart_instructImage', 
        image='instruction_images/selfInitiate_nextTrial.png', mask=None, anchor='center',
        ori=0.0, pos=(0, -0.2), draggable=False, size=(0.25, 0.25),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-1.0)
    selfStart_continueText = visual.TextStim(win=win, name='selfStart_continueText',
        text='<press SPACE to continue>',
        font='Arial',
        pos=(0, -0.4), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-2.0);
    selfStart_keyResp = keyboard.Keyboard(deviceName='defaultKeyboard')
    
    # --- Initialize components for Routine "Practice_startScreen" ---
    practiceStart_text = visual.TextStim(win=win, name='practiceStart_text',
        text='We will start with a few practice trials so you can understand the structure of the experiment and how to make responses.\n\n<press SPACE to continue>',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    report_fix = visual.Rect(
        ...
        colorSpace='rgb', lineColor='black', fillColor='black',
        opacity=None, depth=-1.0, interpolate=True)
    reportCircle_image = visual.ImageStim(
        win=win,
        name='reportCircle_image', 
        image='stimuli/report_circle.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(0.4558, 0.4558),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
    ...
    # -----------------------------
    # RNG: stable per (participant, session)
    # -----------------------------
    participant_number = expInfo['participant']
    session_number = expInfo['session']
    
    sub_int = int(''.join([c for c in str(participant_number) if c.isdigit()]) or 0)
    ses_int = int(''.join([c for c in str(session_number) if c.isdigit()]) or 0)
    
    # deterministic but different across sessions
    seed = sub_int * 1000 + ses_int + 12345
    ...
    def make_balanced_color_trials(
        n_trials, colors, k_per_trial, rng,
    ...
    def write_design_csv(path, rows):
        fieldnames = ["trial", "phase", "trial_n"]
    ...
    # -----------------------------
    if _os.path.exists(practice_csv) and _os.path.exists(main_csv):
        print(f"Design matrices already exist for sub-{participant_number} ses-{session_number}. Skipping generation.")
    else:
        print(f"Generating design matrices for sub-{participant_number} ses-{session_number}...")
        ...
        for t, cols in enumerate(practice_colors):
            ...
            row = {"trial": t, "trial_n": t + 1, "phase": "practice"}  # trial_n starts at 1
            for i in range(N_COLORS_PER_TRIAL):
        ...
        for t, cols in enumerate(main_colors):
            ...
            row = {"trial": t, "trial_n": t + 1, "phase": "main"}  # trial_n starts at 1
            for i in range(N_COLORS_PER_TRIAL):
                row[f"item{i}_color"] = cols[i]
                row[f"item{i}_x"] = float(xy[i, 0])
                row[f"item{i}_y"] = float(xy[i, 1])
    ...
    practice_list = _os.path.join(expDir, 'subject_designs', f'sub-{participant_number}', f'ses-{session_number}', 'practice', 'practice_design.csv')
    
    main_list = _os.path.join(expDir, 'subject_designs', f'sub-{participant_number}', f'ses-{session_number}', 'main', 'main_design.csv')
    thisExp.nextEntry()
    ...
    # --- Prepare to start Routine "selfStart_nextTrial" ---
    # create an object to store info about Routine selfStart_nextTrial
    selfStart_nextTrial = data.Routine(
        name='selfStart_nextTrial',
        components=[selfStart_instructText, selfStart_instructImage, selfStart_continueText, selfStart_keyResp],
    )
    selfStart_nextTrial.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # create starting attributes for selfStart_keyResp
    selfStart_keyResp.keys = []
    selfStart_keyResp.rt = []
    _selfStart_keyResp_allKeys = []
    # store start times for selfStart_nextTrial
    selfStart_nextTrial.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    selfStart_nextTrial.tStart = globalClock.getTime(format='float')
    selfStart_nextTrial.status = STARTED
    thisExp.addData('selfStart_nextTrial.started', selfStart_nextTrial.tStart)
    selfStart_nextTrial.maxDuration = None
    # keep track of which components have finished
    selfStart_nextTrialComponents = selfStart_nextTrial.components
    for thisComponent in selfStart_nextTrial.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "selfStart_nextTrial" ---
    thisExp.currentRoutine = selfStart_nextTrial
    selfStart_nextTrial.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # *selfStart_instructText* updates
        
        # if selfStart_instructText is starting this frame...
        if selfStart_instructText.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            selfStart_instructText.frameNStart = frameN  # exact frame index
            selfStart_instructText.tStart = t  # local t and not account for scr refresh
            selfStart_instructText.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(selfStart_instructText, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'selfStart_instructText.started')
            # update status
            selfStart_instructText.status = STARTED
            selfStart_instructText.setAutoDraw(True)
        
        # if selfStart_instructText is active this frame...
        if selfStart_instructText.status == STARTED:
            # update params
            pass
        
        # *selfStart_instructImage* updates
        
        # if selfStart_instructImage is starting this frame...
        if selfStart_instructImage.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            selfStart_instructImage.frameNStart = frameN  # exact frame index
            selfStart_instructImage.tStart = t  # local t and not account for scr refresh
            selfStart_instructImage.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(selfStart_instructImage, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'selfStart_instructImage.started')
            # update status
            selfStart_instructImage.status = STARTED
            selfStart_instructImage.setAutoDraw(True)
        
        # if selfStart_instructImage is active this frame...
        if selfStart_instructImage.status == STARTED:
            # update params
            pass
        
        # *selfStart_continueText* updates
        
        # if selfStart_continueText is starting this frame...
        if selfStart_continueText.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            selfStart_continueText.frameNStart = frameN  # exact frame index
            selfStart_continueText.tStart = t  # local t and not account for scr refresh
            selfStart_continueText.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(selfStart_continueText, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'selfStart_continueText.started')
            # update status
            selfStart_continueText.status = STARTED
            selfStart_continueText.setAutoDraw(True)
        
        # if selfStart_continueText is active this frame...
        if selfStart_continueText.status == STARTED:
            # update params
            pass
        
        # *selfStart_keyResp* updates
        waitOnFlip = False
        
        # if selfStart_keyResp is starting this frame...
        if selfStart_keyResp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            selfStart_keyResp.frameNStart = frameN  # exact frame index
            selfStart_keyResp.tStart = t  # local t and not account for scr refresh
            selfStart_keyResp.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(selfStart_keyResp, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'selfStart_keyResp.started')
            # update status
            selfStart_keyResp.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(selfStart_keyResp.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(selfStart_keyResp.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if selfStart_keyResp.status == STARTED and not waitOnFlip:
            theseKeys = selfStart_keyResp.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            _selfStart_keyResp_allKeys.extend(theseKeys)
            if len(_selfStart_keyResp_allKeys):
                selfStart_keyResp.keys = _selfStart_keyResp_allKeys[-1].name  # just the last key pressed
                selfStart_keyResp.rt = _selfStart_keyResp_allKeys[-1].rt
                selfStart_keyResp.duration = _selfStart_keyResp_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
        ...
        if thisExp.status == PAUSED:
            pauseExperiment(
                ...
                win=win, 
                timers=[routineTimer, globalClock], 
                currentRoutine=selfStart_nextTrial,
            )
            # skip the frame we paused on
            continue
        ...
        # has a Component requested the Routine to end?
        if not continueRoutine:
            selfStart_nextTrial.forceEnded = routineForceEnded = True
        # has the Routine been forcibly ended?
        if selfStart_nextTrial.forceEnded or routineForceEnded:
            break
        # has every Component finished?
        continueRoutine = False
        for thisComponent in selfStart_nextTrial.components:
    ...
    # --- Ending Routine "selfStart_nextTrial" ---
    for thisComponent in selfStart_nextTrial.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for selfStart_nextTrial
    selfStart_nextTrial.tStop = globalClock.getTime(format='float')
    selfStart_nextTrial.tStopRefresh = tThisFlipGlobal
    thisExp.addData('selfStart_nextTrial.stopped', selfStart_nextTrial.tStop)
    # check responses
    if selfStart_keyResp.keys in ['', [], None]:  # No response was made
        selfStart_keyResp.keys = None
    thisExp.addData('selfStart_keyResp.keys',selfStart_keyResp.keys)
    if selfStart_keyResp.keys != None:  # we had a response
        thisExp.addData('selfStart_keyResp.rt', selfStart_keyResp.rt)
        thisExp.addData('selfStart_keyResp.duration', selfStart_keyResp.duration)
    thisExp.nextEntry()
    # the Routine "selfStart_nextTrial" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisPractice_trial in practice_trials:
        ...
        whole_report = data.Routine(
            name='whole_report',
            components=[report_fix, reportCircle_image, reportCircle, mouse, cursorDot],
        )
        ...
        # Set option colors to THIS TRIAL’S colors
        ...
        # ---- Click accept zone around response wheel ----
        CLICK_BAND_INNER_FRAC = 0.00
        CLICK_BAND_OUTER_FRAC = 1.05
        
        # ---- Static visual cue settings ----
        ADVANCE_LINEWIDTH = 6
        ADVANCE_LINECOLOR = "red"
        
        # Save original fixation appearance
        _fix_orig_lineColor = report_fix.lineColor
        _fix_orig_fillColor = report_fix.fillColor
        _fix_orig_opacity = report_fix.opacity
        _fix_orig_lineWidth = getattr(report_fix, "lineWidth", None)
        ...
        while continueRoutine:
            ...
            # ---- Static red outline on fixation when waiting ----
            if waiting_for_advance:
                report_fix.lineColor = ADVANCE_LINECOLOR
                report_fix.opacity = 1.0
                if getattr(report_fix, "lineWidth", None) is not None:
                    report_fix.lineWidth = ADVANCE_LINEWIDTH
            else:
                report_fix.lineColor = _fix_orig_lineColor
                report_fix.fillColor = _fix_orig_fillColor
                report_fix.opacity = _fix_orig_opacity
                if _fix_orig_lineWidth is not None:
                    report_fix.lineWidth = _fix_orig_lineWidth
            ...
            # -----------------------------
            # click handling (edge-detected)
            # -----------------------------
            left = mouse.getPressed()[0]
            just_clicked = (left and not prev_left)
            prev_left = left
            
            if just_clicked:
            
                if waiting_for_advance:
                    if report_fix.contains(mouse):
                        continueRoutine = False
                    ...
                    inner = CLICK_BAND_INNER_FRAC * r
                    outer = CLICK_BAND_OUTER_FRAC * r
                    ...
                    if not (inner <= dist <= outer):
                        ...
                        resp_by_order_deg.append(float(rad_to_deg(ang)))
                        true_by_order_deg.append(float(rad_to_deg(true_ang)))
                        ...
                        if all(reported):
                            ...
                            phase = "wait_for_advance"
            
                            # Hide tray to emphasize advance step
                            for d in dots:
                                d.setAutoDraw(False)
            ...
            # *reportCircle_image* updates
            
            # if reportCircle_image is starting this frame...
            if reportCircle_image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                reportCircle_image.frameNStart = frameN  # exact frame index
                reportCircle_image.tStart = t  # local t and not account for scr refresh
                reportCircle_image.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(reportCircle_image, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'reportCircle_image.started')
                # update status
                reportCircle_image.status = STARTED
                reportCircle_image.setAutoDraw(True)
            
            # if reportCircle_image is active this frame...
            if reportCircle_image.status == STARTED:
                # update params
                pass
